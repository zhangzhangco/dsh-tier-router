...
export async function replaceImages(ctx, settings, messages, signal, cache, stats, log) {
  ...
  // The sidecar call sits on the critical path (it runs before the turn is
  // routed), so it is bounded: a hung vision provider must not stall the turn.
  const visionTimeoutMs = (() => {
    const configured = Number(settings.visionTimeoutMs)
    return Number.isFinite(configured) && configured > 0 ? configured : VISION_TIMEOUT_MS
  })()

  /**
   * Cached evidence for one attachment, governed by `visionCacheTtl` — the
   * setting is the single authority for this lifetime. A cache exposing
   * `getWithAge` hands over the raw entry so the age check happens here; a
   * plain get/set cache (tests, embedders) enforces its own TTL instead.
   */
  const readCached = (id) => {
    if (cacheTtlMs <= 0) return undefined
    if (typeof cache?.getWithAge === 'function') {
      const entry = cache.getWithAge(id)
      if (entry === undefined) return undefined
      return Date.now() - entry.at <= cacheTtlMs ? entry.value : undefined
    }
    return typeof cache?.get === 'function' ? cache.get(id) : undefined
  }
  ...
  await Promise.all(pending.map(async (id) => {
    ...
    try {
      const raw = await callVisionModel(ctx, vision, attachmentBy.get(id), signal, visionTimeoutMs)
...
/**
 * Default budget for one vision sidecar call. Generous, because the configured
 * vision model may be a CLI adapter (Codex takes tens of seconds per call), but
 * finite: the call runs BEFORE the turn is routed, so an unbounded one hangs the
 * whole request.
 */
 ...
 */
async function callVisionModel(ctx, vision, attachment, signal, timeoutMs = VISION_TIMEOUT_MS) {
  const config = {
  ...
  // Abort the underlying call on timeout (killing a spawned CLI rather than
  // merely abandoning it), while still honouring the caller's own signal.
  ...
  const budget = Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : VISION_TIMEOUT_MS
  // Bell and braces: abort the underlying call (killing a spawned CLI) AND race
  // it, so the turn proceeds even if an adapter ignores the abort signal.
  let timer
  const work = (async () => {
    const prepared = await ctx.llm.prepareCall(config, controller.signal)
  ...
  // Never let a late failure surface as an unhandled rejection.
  work.catch(() => {})
  const expiry = new Promise((_resolve, reject) => {
    timer = setTimeout(() => {
      controller.abort()
      reject(new Error(`vision analysis timed out after ${budget}ms (raise visionTimeoutMs if the vision model needs longer)`))
    }, budget)
  })
  try {
    return await Promise.race([work, expiry])
  } catch (error) {
    if (controller.signal.aborted === true && signal?.aborted !== true && !String(error?.message ?? '').includes('timed out')) {
      throw new Error(`vision analysis timed out after ${budget}ms (raise visionTimeoutMs if the vision model needs longer)`)
    }
    throw error
  } finally {
    clearTimeout(timer)
