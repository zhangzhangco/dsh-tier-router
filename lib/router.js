...
export function estimatePromptTokens(messages, options = {}) {
  ...
  for (const message of Array.isArray(messages) ? messages : []) {
    for (const block of Array.isArray(message?.content) ? message.content : []) {
      ...
      if (block.type === 'text' && typeof block.text === 'string') total += textTokens(block.text)
      else if (block.type === 'image') total += perImage
      else total += 64
    }
...
export class TierRouterAdapter extends LlmAdapter {
  ...
  async contextWindowOf(provider, model) {
    ...
    if (this.contextWindows.has(key)) return this.contextWindows.get(key)
    let window
    try {
    ...
    this.contextWindows.set(key, window)
    return window
  }
  ...
  async resolveChain(options) {
     ...
     * routable request into "no route".
     */
    ...
    if (hasImage) {
      ...
      return {
        ...
        estimate, skipped: vision.skipped,
      }
    }
    ...
    return {
      ...
      estimate, skipped: guardedChain.skipped,
    }
  }
