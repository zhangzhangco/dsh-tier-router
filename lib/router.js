...
import { buildRoutingState, routingStateBounded, routingStateInput, isHumanMessage, isToolResultMessage, blocksText } from './routing-state.js'
...
import {
  JEV_BACKOFF_MS, formatJevProbabilities, jevIdentity, requestJevTier, resolveJevKey,
} from './jev.js'
...
export function createStats() {
  ...
  return {
     ...
     *   level: `heuristic`; `llm`, `llm-cache`, `llm-timeout`, `llm-error`,
     *   `llm-unavailable`; or `jev`, `jev-cache`, `jev-low-confidence`,
     *   `jev-timeout`, `jev-auth`, `jev-rate-limit`, `jev-error`,
     *   `jev-unavailable`.
...
export class TierRouterAdapter extends LlmAdapter {
  constructor(ctx, getSettings, options = {}) {
    ...
    // Jev (TypeSafe) transport + credential seams, injected by tests. The key
    // resolvers default to the environment and the local TypeSafe SDK's key
    // file; nothing here is logged.
    this.fetchImpl = options.fetchImpl ?? globalThis.fetch
    this.jevKeyDeps = options.jevKeyDeps ?? {}
    // A refused key (401) or an exhausted rate limit is not retried on every
    // turn: the settings card already reported it, and paying the round trip
    // while it is broken is pure latency. Cleared by the next good answer.
    this.jevBackoffUntil = 0
    this.jevBackoffSource = ''
    this.jevBackoffDetail = ''
  ...
  async resolveChain(options) {
    ...
    // Nearest tier first (`tierFallbackOrder`), so an easy request the local
    // model could not take lands on `normal` instead of escalating straight to
    // `hard` — the route that is both the most expensive and the one that runs
    // out of quota.
    for (const tier of tierFallbackOrder(level)) {
  ...
  async classifyRequest(state, settings, signal) {
    ...
    // The threshold is a setting because the built-in 3 is unreachable for real
    // work: on 213 measured requests 79% scored exactly 0, so only 3 and 1 differ.
    const heuristic = classifyDifficulty(text, { hardScore: settings.hardScore })
    ...
    if (/^(继续|你继续|改吧|修|继续吧|continue|go on)[。.!！\s]*$/i.test(text)
      ...
      const previous = classifyDifficulty(prior, { hardScore: settings.hardScore })
    ...
    /**
     * A deterministic fact no text classifier reliably sees: the SAME
     * structured tool failure repeated for the same operation means the model
     * currently in play cannot finish this task. Applied as a floor above every
     * classifier's verdict — the whole point of escalating is that the model
     * already failed, so a judgement must not talk the router out of it.
     */
    const failureFloor = state.repeatedFailure
      ? 'repeated structured tool failure for the same operation'
      : undefined
    const withFloor = (decision) => (failureFloor !== undefined && decision.level !== 'hard'
      ? { ...decision, level: 'hard', cause: `${failureFloor} (classifier said ${decision.level})` }
      : decision)
    /** Attribute an abstention, keeping the heuristic's own reasoning visible. */
    const abstain = (why, classifier) => withFloor({
      level: heuristic.level,
      cause: `${why} → heuristic: ${heuristic.reasons.join('; ')}`,
      classifier,
    ...
    if (mode === 'jev') {
      const result = await this.classifyWithJev(state, signal)
      const source = String(result.source ?? 'error')
      if (source === 'jev' || source === 'cache') {
        // An answer below the confidence gate is an abstention, not a verdict:
        // with three options and no real signal the top one is a coin flip.
        const min = Number(settings.jevMinConfidence)
        if (Number.isFinite(min) && min > 0 && Number(result.confidence) < min) {
          return abstain(
            `jev abstained (confidence ${Number(result.confidence).toFixed(2)} < ${min})`,
            'jev-low-confidence',
          )
        }
        const spread = formatJevProbabilities(result.probabilities)
        const detail = [`confidence ${Number(result.confidence).toFixed(2)}`,
          spread === '' ? '' : spread].filter((part) => part !== '').join(', ')
        return withFloor({
          level: result.level,
          cause: `jev: ${result.level} (${detail})`,
          classifier: source === 'cache' ? 'jev-cache' : 'jev',
          inputChars: input.length,
          fingerprint: fingerprintOf(input),
        })
      }
      const why = source === 'timeout'
        ? `no answer within ${Number(settings.classifierTimeoutMs) || CLASSIFIER_TIMEOUT_MS}ms`
        : source === 'unavailable' ? 'no API key configured'
          : source === 'auth' ? 'API key rejected (401/403)'
            : source === 'rate-limit' ? 'rate limited by TypeSafe'
              : source === 'aborted' ? 'aborted'
                : `request failed${result.detail === undefined ? '' : `: ${result.detail}`}`
      return abstain(`jev ${why}`, `jev-${source}`)
    }
    if (mode === 'llm') {
      const result = await this.classifyWithLlm(input, signal)
      if (result.level !== undefined) {
        return withFloor({ level: result.level,
      ...
      return abstain(`llm ${why}`, `llm-${source}`)
    }
  ...
  /**
   * Jev (TypeSafe System One) classification, cached and bounded.
   *
   * Jev answers a typed `choice` question instead of generating text, so the
   * tier arrives as a value with a probability per option and a confidence —
   * nothing to parse out of prose. Resolves to an object, never `undefined`, so
   * the caller can report WHY a tier was chosen:
   *
   * - `{ level, confidence, probabilities, source: 'jev' }`   — fresh judgement
   * - `{ level, confidence, probabilities, source: 'cache' }` — reused
   * - `{ source: 'unavailable' }` — no key in settings, the environment or `~/.typesafe/key`
   * - `{ source: 'timeout' }`     — outlived `classifierTimeoutMs`
   * - `{ source: 'auth' }`        — TypeSafe rejected the key (401/403)
   * - `{ source: 'rate-limit' }`  — 429 or 529
   * - `{ source: 'error' }`       — transport failure or an unusable answer
   * - `{ source: 'aborted' }`     — the request was cancelled
   *
   * Costs one HTTPS round trip on the critical path (~0.8s measured to
   * api.typesafe.ai from this machine), and an agent loop re-sends the same
   * turn on every tool step — so the per-input cache and the scheduler's budget
   * both apply exactly as they do to the LLM classifier.
   *
   * @param {object} state - the bounded routing evidence envelope.
   * @param {AbortSignal} [signal] - request cancellation.
   */
  async classifyWithJev(state, signal) {
    const settings = this.config()
    const { key } = resolveJevKey(settings, this.jevKeyDeps)
    const model = String(settings.jevModel ?? '') || undefined
    // The structured envelope goes over the wire as an OBJECT: Jev reads named
    // fields (userTask, agentStep, repeatedFailure, …) directly, where the LLM
    // classifier only ever sees this serialized as one blob of text.
    const bounded = routingStateBounded(state)
    const cacheKey = classificationKey(JSON.stringify(bounded), jevIdentity(model))
    const now = Date.now()
    if (this.jevBackoffUntil > now) {
      return { source: this.jevBackoffSource, detail: this.jevBackoffDetail }
    }
    const classify = async (callSignal) => {
      const result = await requestJevTier({
        ...
        signal: callSignal,
        timeoutMs: Number(settings.classifierTimeoutMs) || CLASSIFIER_TIMEOUT_MS,
        fetchImpl: this.fetchImpl,
      })
      if (result.source === 'auth' || result.source === 'rate-limit') {
        // Stop paying the round trip while the credential or the quota is
        // broken; the decision records the reason, so it is not silent.
        this.jevBackoffUntil = Date.now() + JEV_BACKOFF_MS
        this.jevBackoffSource = result.source
        this.jevBackoffDetail = `HTTP ${result.status ?? ''}`.trim()
        this.log(`tier-router: jev ${result.source} (${result.status ?? 'n/a'}); skipping Jev for ${Math.round(JEV_BACKOFF_MS / 1000)}s`)
      } else if (result.source === 'jev') {
        this.jevBackoffUntil = 0
        this.jevBackoffSource = ''
        this.jevBackoffDetail = ''
      }
    ...
    const budget = Number(settings.classifierTimeoutMs) || CLASSIFIER_TIMEOUT_MS
    return this.classifications.run(cacheKey, classify, { signal, timeoutMs: budget })
  }
