...
export function createStats() {
  ...
  return {
     ...
     * best available identity is the last user message (`sessionId` + its
     * index in the request). Within one agent loop the array only grows by
     * appending assistant/tool messages, so that index is stable; a new human
     * message moves it, and any change — including a drop after compaction or
     * in a fresh session — starts a new turn.
     *
     * @param {string|undefined} key - turn identity; `undefined` always counts.
    ...
    recordTurn(key, level, hasImage) {
      if (key !== undefined && lastTurnKey !== undefined && key === lastTurnKey) return false
      lastTurnKey = key
      turns.total += 1
...
/** Index of the latest user message in a request's message array, or -1. */
export function lastUserMessageIndex(messages) {
  if (!Array.isArray(messages)) return -1
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    const message = messages[i]
    if (message && message.role === 'user' && Array.isArray(message.content)) return i
  }
  return -1
}
 ...
 *
 * The router cannot see the session log, so the turn is identified by the last
 * user message: its session plus its index in the request. Inside one agent
 * loop the array only grows by appending assistant/tool messages, so the index
 * is stable and every tool step maps to the same turn; sending a new message
 * moves it. The `sessionId` prefix keeps two sessions that happen to sit at the
 * same index apart.
 *
 * @param {object} options - the request.
 * @returns {string|undefined} turn key, or undefined when there is no user message.
 */
