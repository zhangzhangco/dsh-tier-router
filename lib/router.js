...
/**
 * Is this the message a human typed?
 *
 * Positive identification, because `role: 'user'` is NOT enough. dsh-llm
 * declares three different user-role producers: the human
 * (`source.kind === 'user'`), tool results (`'tool'`), and plugin-injected
 * context (`'plugin'`, carrying `form: instructions | catalog | snapshot |
 * notice | relay | recall`). The agent loop appends a `snapshot` user message
 * after every turn, so a backwards search that only excludes tool results
 * lands on injected state — which is what made the classifier reply "No
 * explicit request text is shown" and default to `normal`.
 *
 * Hand-built requests (tests, other callers) may carry no source at all, so an
 * absent or unrecognised source falls back to structure: user role and not a
 * tool result. A *recognised* non-human source is always rejected.
 *
