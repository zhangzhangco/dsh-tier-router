...
默认仍是 `heuristic`。选择 `llm` 并在 `llmClassifierProvider/Model` 中指定模型，即启用结合任务与步骤上下文的生成式分类；升级不会自动切换生产分类器。

启发式另外覆盖两个关键词覆盖不到的情形：短的延续指令（`继续`、`continue` 等）继承上一轮任务的档位；同一次工具调用出现结构化重复失败时至少为 `hard`。
